...
class Media {
  ...
  onResize(config?: { screen?: { width: number; height: number }; viewport?: { width: number; height: number } }) {
    if (config?.screen) this.screen = config.screen;
    if (config?.viewport) {
      this.viewport = config.viewport;
      if (this.plane && this.plane.program && (this.plane.program as any).uniforms.uViewportSizes) {
    ...
    this.padding = 2;
    this.width = this.plane ? this.plane.scale.x + this.padding : 0;
...
class App {
  ...
  boundOnResize: (...args: any[]) => void;
  boundOnWheel: (...args: any[]) => void;
  boundOnTouchDown: (...args: any[]) => void;
  boundOnTouchMove: (...args: any[]) => void;
  boundOnTouchUp: (...args: any[]) => void;
  ...
  onResize = () => {
    ...
    const fov = this.camera ? (this.camera.fov * Math.PI) / 180 : 0;
    const height = this.camera ? 2 * Math.tan(fov / 2) * this.camera.position.z : 0;
  ...
  addEventListeners() {
    window.addEventListener('resize', this.boundOnResize as EventListener);
  ...
  destroy() {
    ...
    window.removeEventListener('resize', this.boundOnResize as EventListener);
